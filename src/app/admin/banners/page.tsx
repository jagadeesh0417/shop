...
import { getBanners, saveBanners, Banner } from '@/lib/adminStore';
...
export default function AdminBannersPage() {
  ...
  const addBanner = () => {
    ...
    const updated = [...banners, { id: String(Date.now()), ...form, active: true }];
    saveBanners(updated);
    setBanners(updated);
    ...
    <div className="pt-28 pb-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded ${
                      banner.active ? 'bg-success/20 text-success' : 'bg-error/20 text-error'
                    }`}
                  >
                    {banner.active ? 'Active' : 'Inactive'}
                  </button>
                  <button onClick={() => removeBanner(banner.id)} className="p-1.5 text-text-muted hover:text-error transition-colors">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
          ...
          {banners.length === 0 && (
            <p className="text-center text-text-muted py-12">No banners yet. Add one above.</p>
          )}
        </div>
      </div>
    </div>
